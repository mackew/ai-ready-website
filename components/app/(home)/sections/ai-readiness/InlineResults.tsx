...
/*
  Single, clean implementation of the InlineResults component.
  All previous duplicate blocks were removed.
*/
...
// Temporary mock data – replace with real results in future integration
const mockResults = {
  score: 78,
  grade: "B+",
  llmsTxt: true,
  ...
  return (
    <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="relative">
            ...
            <motion.div
              ...
              style={{
                left: `${((analysisStep + 1) / analysisSteps.length) * 100}%`,
      ...
      {showResults && (
        ...
        >
          {/* Blur wrapper */}
          <div className={isBlurred ? "filter blur-md pointer-events-none" : ""}>
            {/* Score */}
            ...
            >
              {[
                { label: "LLMs.txt", value: mockResults.llmsTxt, icon: FileText },
                { label: "Structured Data", value: mockResults.structuredData, icon: Code },
                { label: "Semantic HTML", value: mockResults.semanticHTML, icon: Globe },
              ].map((item, i) => (
                <motion.div
                  key={item.label}
                  ...
                  className={`relative p-16 rounded-12 border transition-all hover:shadow-md cursor-pointer ${
                    item.value ? "bg-green-50/50 border-green-200" : "bg-red-50/50 border-red-200"
                  }`}
                >
                  <div className="absolute top-12 right-12">
                    {item.value ? (
                      <Check className="w-24 h-24 text-green-600" strokeWidth={3} />
                    ) : (
                      <X className="w-24 h-24 text-red-600" strokeWidth={3} />
                    )}
                  </div>
                  <item.icon className={`w-24 h-24 mb-12 ${item.value ? "text-green-600" : "text-red-600"}`} />
                  <div className="space-y-4">
                    <div className="text-label-medium text-accent-black">{item.label}</div>
                    <div className="text-body-small text-black-alpha-48">
                      {item.value ? "Detected" : "Not found"}
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          {isBlurred && (
            <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-16">
              <p className="text-label-medium text-accent-black">Verify your email to see full results</p>
            </div>
          )}
        </motion.div>
      )}
